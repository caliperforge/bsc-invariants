...
| PancakeSwap v3 | Liquidity events | **P-4** | mint/burn updates `liquidity` by exactly `±delta` for in-range positions | — | ⬜ | M2 |
| PancakeSwap v3 | Per-tick state | **P-5** | `feeGrowthOutside0X128` per tick is conserved across crossings (sum of inside + outside fragments equals global) | — | ⬜ | M2 |
...
| PancakeSwap v3 | Planted-twin CI | — | clean + planted CI pair for P-1, P-2, P-3 (planted leg surfaces `INVARIANT VIOLATED` marker, exits non-zero) | `invariants/planted/` | ⬜ | M2 |
